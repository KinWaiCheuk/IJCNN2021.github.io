...
class Onset_Stack(nn.Module):
    def __init__(self, input_features, model_size, output_features):
        super().__init__()
...
class Combine_Stack(nn.Module):
    def __init__(self, model_size, output_features):
        super().__init__()
        self.sequence_model = sequence_model(output_features * 2, model_size)
        self.linear = nn.Linear(model_size, output_features)
...
class Combine_Stack_with_attn(nn.Module):
    def __init__(self, model_size, output_features, attention_mode, w_size):
        super().__init__()
        self.w_size = w_size
...
#         x, hidden = self.sequence_model(x, hidden)
        
#         a = self.attention(x, encoder_outputs)
# #         a = a.unsqueeze(1)        
# #         print(f'a unsqueeze shape = {a.shape}')
#         weighted = torch.bmm(a, encoder_outputs)
#         x = torch.cat((x, weighted), dim=2)
#         x = self.linear(x)
...
class OnsetsAndFrames_with_fast_local_attn_noLSTM(nn.Module):
    def __init__(self, input_features, output_features, model_complexity=48, log=True, mode='imagewise', spec='Mel', device='cpu', attention_mode='activation', w_size=30, onset_stack=True):
        super().__init__()
...
#         self.offset_stack = Offset_Stack(input_features, model_size, output_features, sequence_model(model_size, model_size))
        if onset_stack==True:
            self.combined_stack = Combine_Stack_with_attn(model_size, output_features, attention_mode, w_size)
...
#         offset_pred = self.offset_stack(mel)
    ...
    def forward_frame_only(self, spec):
        ...
        if self.attention_mode=='onset':
            frame_pred, a = self.combined_stack(activation_pred, None, onset_pred) # Attenting on onset
        elif self.attention_mode=='activation':
            frame_pred, a = self.combined_stack(activation_pred, None, activation_pred) # Attenting on features
        elif self.attention_mode=='spec':
            frame_pred, a = self.combined_stack(activation_pred, None, spec) # Attenting on features
