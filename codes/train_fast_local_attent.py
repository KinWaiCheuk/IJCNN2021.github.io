...
#     os.environ['CUDA_VISIBLE_DEVICES']=str(GPU)
    ...
    logdir = f'runs/fast-local-{attention_mode}-Attention-{w_size}-onset_stack_{onset_stack}-{train_on}-{spec}-{mode}_log-{log}_lr-{learning_rate}-'+ datetime.now().strftime('%y%m%d-%H%M%S')
        
    ex.observers.append(FileStorageObserver.create(logdir)) # saving source code
        
@ex.automain
def train(spec, resume_iteration, train_on, batch_size, sequence_length,w_size, onset_stack, LSTM,
          learning_rate, learning_rate_decay_steps, learning_rate_decay_rate, leave_one_out, label_type,
          clip_gradient_norm, validation_length, refresh, device, reconstruction, epoches, logdir, log, attention_mode): 
    ...
    if resume_iteration is None:
        ...
        model = OnsetsAndFrames_with_fast_local_attn(N_BINS, MAX_MIDI - MIN_MIDI + 1, log=log, mode=mode, spec=spec, device=device, attention_mode=attention_mode, w_size=w_size, onset_stack=onset_stack, LSTM=LSTM)
        model.to(device)
...
#             os.makedirs(logdir, exist_ok=True) # creating the log dir
        ...
        if ep%logging_freq == 0:
            ...
            if 'onset' in label_type and onset_stack==True:
                fig, axs = plt.subplots(2, 2, figsize=(24,4))
                axs = axs.flat
